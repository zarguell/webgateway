...
|---|---|---|
| `src/webgateway/config.py` | Modify (lines 111–118) | Refactor `LLMJudgeConfig` from Ollama to generic OpenAI-compatible |
| `src/webgateway/judge.py` | **Create** | `DecisionCache`, dataclasses, `LLMJudge` class (prompt builder, JSON parser, LLM client, evaluate methods) |
| `src/webgateway/service.py` | Modify (lines 65–87, 116–122, 354–360, 745–806) | Wire judge into constructor, `search()`, `extract()`, `_execute_with_fallback()` |
| `src/webgateway/main.py` | Modify (lines 129–140) | Construct `LLMJudge` when enabled, inject into `GatewayService` |
...
**Files:**
- Modify: `src/webgateway/config.py` (lines 111–118)
...
Replace lines 111–118 in `src/webgateway/config.py`:
...
Run: `source .venv/bin/activate && ruff check src/webgateway/config.py tests/unit/test_judge_config.py`
Expected: No errors

- [ ] **Step 7: Commit**

```bash
git add src/webgateway/config.py pyproject.toml tests/unit/test_judge_config.py
...
**Files:**
- Create: `src/webgateway/judge.py`
...
Expected: FAIL — `webgateway.judge` module doesn't exist
...
Run: `source .venv/bin/activate && ruff check src/webgateway/judge.py tests/unit/test_judge.py`
Expected: No errors

- [ ] **Step 6: Commit**

```bash
git add src/webgateway/judge.py tests/unit/test_judge.py
...
**Files:**
- Modify: `src/webgateway/judge.py` (add LLMJudge class)
...
Append to `src/webgateway/judge.py` (after the `_cache_key` function):
...
**Files:**
- Modify: `src/webgateway/service.py` (lines 65–87, 116–122, 354–360)
...
from webgateway.judge import LLMJudge as LLMJudgeClass
...
In `src/webgateway/service.py`, add import at top (after existing judge-related imports — find the import block around line 11):

Add this import near the top of the file (after the other `from webgateway.` imports):
...
Run: `source .venv/bin/activate && ruff check src/webgateway/service.py tests/unit/test_judge.py`
Expected: No errors

- [ ] **Step 8: Commit**

```bash
git add src/webgateway/service.py tests/unit/test_judge.py
...
**Files:**
- Modify: `src/webgateway/service.py` (lines 745–806)
...
class TestServiceRetryIntegration:
    async def test_judge_called_on_provider_error(self):
        ...
        from webgateway.policy.models import RoutingDecision as RD
        decision = RD(
            policy_matched=None,
            content_type="extract",
            provider="jina",
...
In `src/webgateway/service.py`, replace the `_execute_with_fallback` method (lines 745–806) with:
...
**Files:**
- Modify: `src/webgateway/main.py` (lines 129–140)
...
In `src/webgateway/main.py`, add import after line 34 (`from webgateway.proxy import ProxyResolver`):
...
Run: `source .venv/bin/activate && ruff check src/webgateway/main.py src/webgateway/service.py src/webgateway/judge.py src/webgateway/config.py`
Expected: No errors

- [ ] **Step 7: Commit**

```bash
git add src/webgateway/main.py config.test.yaml tests/unit/test_judge.py
