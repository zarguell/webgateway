---
# WebGateway — Product Requirements Document
**Version:** 0.1-draft
**Date:** 2026-06-16
...
6. **Composable with LiteLLM** — sits below LiteLLM in the stack. LiteLLM handles model/user RBAC above; WebGateway handles fetch routing and data policy below. Joined by `request_id`.
...
- RBAC deferred to LiteLLM layer above — WebGateway keys are service identities, not user identities
...
User / Agent
  ...
  Tool calls: web_search / web_scrape
     ↓
  WebGateway             ← provider routing, proxy, DLP, audit
     ↓
  Search & Scrape Providers
```

WebGateway is a **service identity** in LiteLLM's model — one API key represents the gateway. All user/team/org identity lives in LiteLLM. Cross-layer audit is achieved by passing `X-Request-ID` as a header through the stack and joining on it.
